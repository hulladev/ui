...
function highlightItem(menu: HTMLElement, item: HTMLElement | undefined) {
  menu.querySelector<HTMLElement>("[data-highlighted='true']")?.removeAttribute("data-highlighted")
...
export function connectDropdownMenu(menu: HTMLElement): DropdownMenuController {
  ...
  const onKeyDown = (event: KeyboardEvent) => {
    ...
    const match = searchOrder.find((item) => itemText(item).startsWith(typeahead))
  ...
  const onClick = (event: MouseEvent) => {
    ...
    queueMicrotask(() => {
      if (!event.defaultPrevented && menu.matches(":popover-open")) menu.hidePopover()
    })
  }

  menu.dataset.state = menu.matches(":popover-open") ? "open" : "closed"
  menu.ownerDocument.addEventListener("pointerdown", onDocumentPointerDown, true)
  ...
  return {
    destroy() {
      clearTimeout(typeaheadTimer)
      menu.ownerDocument.removeEventListener("pointerdown", onDocumentPointerDown, true)

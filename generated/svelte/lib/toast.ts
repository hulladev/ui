...
export const pushToast = (input: ToastInput): ToastId => {
  ...
  toastRecords = [record, ...toastRecords.filter((toast) => toast.id !== record.id)]
  emitToasts()
  return record.id
}
...
export const toastClassNames = {
  ...
  toast:
    "group/toast pointer-events-auto relative grid w-full grid-cols-[minmax(0,1fr)_auto] items-start gap-x-3 overflow-hidden rounded-lg border border-[color-mix(in_oklab,var(--toast-accent)_14%,var(--color-border))] bg-[color-mix(in_oklab,var(--toast-accent)_3%,color-mix(in_oklab,var(--color-surface-raised)_86%,transparent))] p-4 pb-[1.625rem] text-foreground shadow-[0_1px_2px_oklch(0_0_0/0.06),0_12px_32px_oklch(0_0_0/0.12),inset_0_1px_color-mix(in_oklab,white_62%,transparent)] backdrop-blur-2xl backdrop-saturate-[1.18] [animation:hulla-toast-in_180ms_cubic-bezier(0.2,0.8,0.2,1)_both] motion-reduce:animate-none dark:bg-[color-mix(in_oklab,var(--toast-accent)_4%,color-mix(in_oklab,var(--color-surface-raised)_84%,transparent))] dark:shadow-[0_2px_4px_oklch(0_0_0/0.28),0_18px_48px_oklch(0_0_0/0.4),inset_0_1px_color-mix(in_oklab,white_10%,transparent)]",

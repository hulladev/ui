...
function elementForSlot<T extends HTMLElement>(root: HTMLElement, slot: string): T | undefined {
...
function hidePopover(surface: PopoverElement) {
  if (!isPopoverOpen(surface) || typeof surface.hidePopover !== "function") return
  try {
    surface.hidePopover()
  } catch {
...
function initialPickerValue(root: HTMLElement): string | null {
  try {
    ...
    return isTimeValue(candidate) && timeWithinBounds(candidate, root.dataset.min, root.dataset.max)
      ? candidate
      : null
  } catch {
    return null
...
export function timeWithinBounds(value: string, min?: string, max?: string): boolean {
  ...
  try {
    ...
    return candidates.filter(
      (candidate): candidate is string =>
        isTimeValue(candidate) && timeWithinBounds(candidate, root.dataset.min, root.dataset.max)
    )
  } catch {
    return []
  }
...
function nearestTimeValue(values: string[], targetMinutes: number): string | null {
  return (
...
function initialDraftValue(values: string[], selectedValue: string | null): string | null {
...
export function timeValueParts(
  ...
  return {
    hour: parts.find((part) => part.type === "hour")?.value ?? String(hour).padStart(2, "0"),
    minute: parts.find((part) => part.type === "minute")?.value ?? String(minute).padStart(2, "0"),
    period: parts.find((part) => part.type === "dayPeriod")?.value,
  }
}

export function timePickerControllerFor(root: HTMLElement): TimePickerController | undefined {
...
export function connectTimePicker(root: HTMLElement): TimePickerController {
  ...
  const minuteDisplay = elementForSlot<HTMLInputElement>(root, "time-picker-minute")
  ...
  function hourCycle(): "h11" | "h12" | "h23" | "h24" {
    ...
    return candidate === "h11" || candidate === "h12" || candidate === "h24" ? candidate : "h23"
  }
  ...
  function maximumMinutes() {
    return isTimeValue(root.dataset.max) ? timeToMinutes(root.dataset.max) : 24 * 60 - 1
  }

  function stepMinutes() {
    const candidate = Number(root.dataset.step)
    return Number.isFinite(candidate) ? Math.min(24 * 60, Math.max(1, Math.floor(candidate))) : 30
  }

  function clampedValue(minutes: number): string {
    return minutesToTime(Math.min(maximumMinutes(), Math.max(minimumMinutes(), minutes)))
  }

  function setActivePart(part: TimePickerPart) {
    activePart = part
    root.querySelectorAll<HTMLElement>("[data-slot='time-picker-part']").forEach((element) => {
      element.dataset.active = element.dataset.timePickerPart === activePart ? "true" : "false"
    })
  }

  function update() {
    const formatted = selectedValue
      ? formatTimeValue(selectedValue, root.dataset.locale || undefined, hourCycle())
    ...
    const parts = timeValueParts(currentDraft, root.dataset.locale || undefined, hourCycle())
    ...
    root.querySelectorAll<HTMLButtonElement>("[data-time-picker-action]").forEach((button) => {
      ...
      if (action === "period") {
        ...
        const hasPeriod = period === "am" ? minimumMinutes() < 12 * 60 : maximumMinutes() >= 12 * 60
        button.disabled = !hasPeriod
        const selectedPeriod = Number(currentDraft.slice(0, 2)) < 12 ? "am" : "pm"
  ...
  function select(value: string | undefined, close = false) {
    if (!value || !timeWithinBounds(value, root.dataset.min, root.dataset.max)) return
    ...
    if (changed) {
      root.dispatchEvent(
        new CustomEvent<TimePickerValueChangeDetail>(TIME_PICKER_VALUE_CHANGE_EVENT, {
          bubbles: true,
  ...
  function manualCandidate(): string | null {
    ...
    hourDisplayControl.setAttribute("aria-invalid", String(!hourIsValid || boundsAreInvalid))
    minuteDisplayControl.setAttribute("aria-invalid", String(!minuteIsValid || boundsAreInvalid))
  ...
  function focusInvalidField() {
    const invalid =
      controlsControl.querySelector<HTMLInputElement>("[aria-invalid='true']") ?? hourDisplayControl
  ...
  function handleClick(event: Event) {
    ...
    if (action === "period" && (button.dataset.period === "am" || button.dataset.period === "pm")) {
      if (!applyManualValue()) {
        focusInvalidField()
        return
      }
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    if (event.target !== controlsControl || !draftValue || values.length === 0) return
    ...
    switch (event.key) {
      ...
      case "ArrowRight":
        setActivePart(
          parts[Math.min(parts.length - 1, Math.max(0, activeIndex + (rtl ? -1 : 1)))] ?? "hour"
        )
        break
      case "ArrowLeft":
        setActivePart(
  ...
  const controller: TimePickerController = {
    ...
    refresh() {
      ...
      if (selectedValue && !timeWithinBounds(selectedValue, root.dataset.min, root.dataset.max)) {
        selectedValue = null
      }
      draftValue = initialDraftValue(values, selectedValue)
      update()
    ...
    setValue(value: string | null) {
      if (destroyed) return
      selectedValue =
        isTimeValue(value) && timeWithinBounds(value, root.dataset.min, root.dataset.max)
          ? value
          : null
      draftValue = initialDraftValue(values, selectedValue)
      update()

...
export function connectFloatingLayer(
  ...
  if (typeof surface.showPopover !== "function" || typeof surface.hidePopover !== "function") {
    surface.dataset.state = "closed"
  ...
  const updatePosition = async () => {
    const { x, y, placement } = await computePosition(trigger, surface, {
      middleware: [offset(options.offset), flip({ padding: 8 }), shift({ padding: 8 })],
  ...
  const show = () => {
    ...
    document.querySelectorAll<HTMLElement>("[data-hulla-floating-layer]").forEach((activeLayer) => {
      if (
        activeLayer === surface ||
  ...
  const handleFocusOut = () => {
    queueMicrotask(() => {
      ...
      if (activeElement && (trigger.contains(activeElement) || surface.contains(activeElement))) {
        clearCloseTimer()
        return
      }

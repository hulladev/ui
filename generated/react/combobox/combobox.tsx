...
type ComboboxBaseProps = Omit<ComponentPropsWithRef<"div">, "defaultValue"> & {
...
export function Combobox(props: ComboboxProps) {
  ...
  const controllerRef = useRef<ComboboxController>(null)
  ...
  useEffect(() => {
    ...
    const handleValueChange = (event: Event) => {
      ...
      if (Array.isArray(nextValue)) {
        ;(onValueChangeRef.current as MultipleComboboxProps["onValueChange"])?.(nextValue)
      } else {
        ;(onValueChangeRef.current as SingleComboboxProps["onValueChange"])?.(nextValue)
      }

      if (valueRef.current !== undefined) {
        queueMicrotask(() => controller.setValue(valueRef.current as ComboboxValue))
      }
    }

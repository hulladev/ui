...
function thumb(root: HTMLElement, selector: string): HTMLInputElement | undefined {
  return Array.from(root.querySelectorAll<HTMLInputElement>(selector)).find(
    (candidate) => candidate.closest<HTMLElement>("[data-slot='range-slider']") === root
  )
}
...
export function connectRangeSlider(root: HTMLElement): RangeSliderController {
  ...
  function refresh(changedThumb?: HTMLInputElement) {
    ...
    let start = clamp(finiteValue(minimumThumb.valueAsNumber, minimum), minimum, maximum)
    let end = clamp(finiteValue(maximumThumb.valueAsNumber, maximum), minimum, maximum)
    ...
    root.style.setProperty("--range-slider-size", `${endPosition - startPosition}%`)

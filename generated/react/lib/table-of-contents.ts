...
export function connectTableOfContents(nav: HTMLElement) {
  ...
  const list = nav.querySelector<HTMLOListElement>("[data-slot=table-of-contents-list]")!
  ...
  function update() {
    ...
    const visible = sections.filter(
      (section) => section.getClientRects().length && !section.closest("[hidden],[inert]")
    )
    ...
    for (const section of visible) {
      ...
      if (section.getBoundingClientRect().top <= top + offset + 1) active = section
    }
    ...
    if (last) {
      ...
      if (atBottom && last.getBoundingClientRect().top < viewportBottom) active = last
    }
  ...
  observer.observe(doc.body, {
    ...
    subtree: true,
    attributes: true,
    attributeFilter: ["id", "data-toc-label", "data-toc-for", "data-toc-offset", "hidden", "inert"],
  })

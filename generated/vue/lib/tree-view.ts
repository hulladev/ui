...
function directGroup(item: HTMLElement): HTMLElement | undefined {
  return Array.from(item.children).find(
    (child): child is HTMLElement => child instanceof HTMLElement && child.matches(groupSelector)
  )
}
...
function childItems(item: HTMLElement, root: HTMLElement): HTMLElement[] {
  ...
  return Array.from(group.children).filter(
    (child): child is HTMLElement =>
      child instanceof HTMLElement && child.matches(itemSelector) && belongsToRoot(child, root)
  )
}
...
function visibleItems(root: HTMLElement): HTMLElement[] {
  return items(root).filter((item) => !isDisabled(item) && isVisible(item, root))
}
...
export function connectTreeView(root: HTMLElement): TreeViewController {
  ...
  function enabledValue(candidate: string): string {
    return items(root).some((item) => !isDisabled(item) && itemValue(item) === candidate)
      ? candidate
      : ""
  }
  ...
  function setExpanded(item: HTMLElement, expanded: boolean, emit = false) {
    ...
    if (emit) {
      root.dispatchEvent(
        new CustomEvent<TreeViewExpandedChangeDetail>(TREE_VIEW_EXPANDED_CHANGE_EVENT, {
          bubbles: true,
  ...
  function updateSelection() {
    ...
    availableItems.forEach((item) => {
      const selected = mode === "single" && itemValue(item) !== "" && itemValue(item) === value

      if (mode === "single") item.setAttribute("aria-selected", String(selected))
  ...
  function refresh() {
    ...
    const currentTabStop = availableItems.find(
      (item) => item.tabIndex === 0 && !isDisabled(item) && isVisible(item, root)
    )
    const selectedItem = availableItems.find(
      (item) => item.getAttribute("aria-selected") === "true" && !isDisabled(item)
    )
  ...
  function onKeyDown(event: KeyboardEvent) {
    ...
    if (!currentItem || !belongsToRoot(currentItem, root) || isDisabled(currentItem)) return
    ...
    if (event.key === "Home" || event.key === "End") {
      event.preventDefault()
      moveFocus(event.key === "Home" ? availableItems[0]! : availableItems.at(-1)!)
      return
    }

    if (event.key === "ArrowRight") {
      event.preventDefault()
      const children = childItems(currentItem, root).filter((item) => !isDisabled(item))
    ...
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault()
      select(currentItem)
      if (directGroup(currentItem)) setExpanded(currentItem, !isExpanded(currentItem), true)
    }
  }

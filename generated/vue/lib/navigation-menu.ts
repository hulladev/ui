...
function itemTrigger(item: HTMLElement): HTMLButtonElement | undefined {
  return Array.from(item.querySelectorAll<HTMLButtonElement>(triggerSelector)).find(
    (trigger) => trigger.closest(itemSelector) === item
  )
}
...
function enabledTriggers(root: HTMLElement): HTMLButtonElement[] {
  return menuItems(root)
    .map(itemTrigger)
    .filter(
      (trigger): trigger is HTMLButtonElement => trigger !== undefined && !isDisabled(trigger)
    )
}

function focusableContent(content: HTMLElement | undefined): HTMLElement | undefined {
  return (
...
function positionContent(root: HTMLElement, content: HTMLElement) {
  ...
  if (rect.right > viewportWidth - gutter) shift = viewportWidth - gutter - rect.right
  if (rect.left + shift < gutter) shift += gutter - (rect.left + shift)

  content.style.setProperty("--navigation-menu-shift-x", `${Math.round(shift)}px`)
}

export function connectNavigationMenu(root: HTMLElement): NavigationMenuController {
  ...
  function updateState() {
    menuItems(root).forEach((item, index) => {
      ...
      const open = value !== "" && value === currentValue && Boolean(trigger && content)
  ...
  function onClick(event: MouseEvent) {
    ...
    if (trigger && belongsToRoot(trigger, root) && !isDisabled(trigger)) {
      ...
      if (value === currentValue && (wasOpenOnPointerDown || event.detail === 0)) close()
      else open(value)
      return
    }
  ...
  function onKeyDown(event: KeyboardEvent) {
    ...
    if (trigger && belongsToRoot(trigger, root) && !isDisabled(trigger)) {
      ...
      if (event.key === "ArrowDown" || event.key === "Enter" || event.key === " ") {
        ...
        const value = item ? itemValue(item) : ""
        open(value)
        queueMicrotask(() => focusableContent(item ? itemContent(item) : undefined)?.focus())
        return
      }
    }
  ...
  function onDocumentPointerDown(event: PointerEvent) {
    ...
    if (root.contains(target)) {
      const trigger =
        target instanceof Element ? target.closest<HTMLButtonElement>(triggerSelector) : null
      pointerDownValue = trigger && belongsToRoot(trigger, root) ? currentValue : ""
      return
    }

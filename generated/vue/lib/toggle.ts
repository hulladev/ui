...
function groupToggles(root: HTMLElement): HTMLButtonElement[] {
  return Array.from(root.querySelectorAll<HTMLButtonElement>(toggleSelector)).filter((toggle) =>
    belongsToGroup(toggle, root)
  )
}
...
export function connectToggle(toggle: HTMLButtonElement): ToggleController {
  const onClick = () => {
    if (isDisabled(toggle) || toggle.closest("[data-slot='toggle-group']")) return
...
export function connectToggleGroup(root: HTMLElement): ToggleGroupController {
  ...
  function normalizedValue(nextValue: ToggleGroupValue): ToggleGroupValue {
    ...
    if (type() === "multiple") {
      return normalizeMultipleValue(nextValue).filter((item) => available.has(item))
    }
  ...
  function refresh() {
    ...
    const nextType = type()
    if (root.dataset.orientation !== orientation) root.dataset.orientation = orientation
  ...
  function activate(toggle: HTMLButtonElement) {
    ...
    updateState()
    root.dispatchEvent(
      new CustomEvent<ToggleGroupValueChangeDetail>(TOGGLE_GROUP_VALUE_CHANGE_EVENT, {
        bubbles: true,
        detail: { value },
      })
    )
  ...
  observer.observe(root, {
    attributeFilter: ["aria-disabled", "aria-pressed", "data-type", "disabled", "value"],
    attributes: true,
    childList: true,
    subtree: true,
  })

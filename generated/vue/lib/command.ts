...
function itemElements(root: HTMLElement): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>(itemSelector)).filter((item) =>
    belongsToRoot(item, root)
  )
}

function groupElements(root: HTMLElement): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>("[data-slot='command-group']")).filter(
    (group) => belongsToRoot(group, root)
  )
}

function emptyElements(root: HTMLElement): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>("[data-slot='command-empty']")).filter(
...
function itemSearchText(item: HTMLElement): string {
  return [item.dataset.value, item.dataset.textValue, item.dataset.keywords, item.textContent]
    .filter(Boolean)
    .join(" ")
}
...
export function connectCommand(root: HTMLElement): CommandController {
  const inputElement = root.querySelector<HTMLInputElement>("[data-slot='command-input']")
  const listElement = root.querySelector<HTMLElement>("[data-slot='command-list']")
  ...
  if (!list.id) {
    commandListId += 1
    list.id = root.id ? `${root.id}-list` : `hulla-command-list-${commandListId}`
  }
  ...
  function filterItems() {
    ...
    items().forEach((item) => {
      item.hidden = shouldFilter && query !== "" && !normalize(itemSearchText(item)).includes(query)
    })

    groupElements(root).forEach((group) => {
      const groupItems = Array.from(group.querySelectorAll<HTMLElement>(itemSelector)).filter(
        (item) => item.closest("[data-slot='command-group']") === group
      )
    ...
    setActive(activeItem && enabled.includes(activeItem) ? activeItem : enabled[0])
  ...
  function refresh() {
    ...
    groupElements(root).forEach((group, index) => {
      ...
      ).find((candidate) => candidate.closest("[data-slot='command-group']") === group)
      if (!label) return
  ...
  function onPointerMove(event: PointerEvent) {
    ...
    const item = target.closest<HTMLElement>(itemSelector)
    if (item && belongsToRoot(item, root) && !item.hidden && !isDisabled(item)) {
      setActive(item)
    }
  }

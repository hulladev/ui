...
function tabElements(root: HTMLElement): HTMLButtonElement[] {
  return Array.from(root.querySelectorAll<HTMLButtonElement>(tabSelector)).filter((tab) =>
    belongsToRoot(tab, root)
  )
}

function panelElements(root: HTMLElement): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>(panelSelector)).filter((panel) =>
    belongsToRoot(panel, root)
  )
}

function isDisabled(tab: HTMLButtonElement): boolean {
  return (
    tab.disabled || tab.dataset.disabled === "true" || tab.getAttribute("aria-disabled") === "true"
  )
}
...
export function connectTabs(root: HTMLElement): TabsController {
  ...
  function updateSelection() {
    ...
    availableTabs.forEach((tab, index) => {
      ...
      const panel = availablePanels.find((candidate) => tabValue(candidate) === value)
  ...
  function refresh() {
    const tabList = Array.from(root.querySelectorAll<HTMLElement>("[data-slot='tabs-list']")).find(
      (candidate) => belongsToRoot(candidate, root)
    )
    ...
    selectedValue = normalizedValue(selectedValue) || tabValue(enabledTabs()[0] ?? root)
    updateSelection()
  }
  ...
  function onKeyDown(event: KeyboardEvent) {
    ...
    if (!currentTab || !belongsToRoot(currentTab, root) || isDisabled(currentTab)) return
    ...
    if (root.dataset.activationMode === "manual" && (event.key === "Enter" || event.key === " ")) {
      event.preventDefault()
      activate(currentTab)
    }
  }

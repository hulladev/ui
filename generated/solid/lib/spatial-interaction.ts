...
const matchingHandle = (
  ...
  if (!handle || handle.closest(rootSelector) !== root || isDisabledHandle(handle)) return
  return handle
}
...
export function resolveDragPosition(
  ...
  const x = axis === "y" ? start.x : clamp(start.x + delta.x, bounds.minX, bounds.maxX)
  const y = axis === "x" ? start.y : clamp(start.y + delta.y, bounds.minY, bounds.maxY)
  return { x, y }
}
...
const dragBoundsFor = (
  ...
  const boundaryRect = boundaryRectFor(root, boundary, boundarySelector, boundaryPadding)
...
export function connectDraggable(
  ...
  const emit = (phase: keyof typeof dragEventNames, detail: DragEventDetail) => {
  ...
  const finishPointer = (canceled: boolean, client?: SpatialPosition) => {
    ...
    emit("end", dragDetail(position, completed.startPosition, "pointer", canceled))
  }

  const onPointerDown = (event: PointerEvent) => {
    const handle = matchingHandle(event, root, draggableHandleSelector, draggableRootSelector)
    if (
      !handle ||
      active ||
      !event.isPrimary ||
    ...
    active = {
      bounds: dragBoundsFor(root, boundary, boundarySelector, boundaryPadding, startPosition),
      handle,
  ...
  const onKeyDown = (event: KeyboardEvent) => {
    ...
    const position = resolveDragPosition(
      ...
      axis,
      dragBoundsFor(root, boundary, boundarySelector, boundaryPadding, startPosition)
    )
    root.dataset.state = "dragging"
    handle.dataset.state = "dragging"
...
const resizeLimitsFor = (
  ...
  const limits = {
    maxHeight: computed ? readMaximum(computed.maxHeight) : Number.POSITIVE_INFINITY,
    maxWidth: computed ? readMaximum(computed.maxWidth) : Number.POSITIVE_INFINITY,
  ...
  return {
    ...limits,
    maxHeight: Math.max(limits.minHeight, Math.min(limits.maxHeight, boundaryMaxHeight)),
    maxWidth: Math.max(limits.minWidth, Math.min(limits.maxWidth, boundaryMaxWidth)),
  }
}
...
export function connectResizable(
  ...
  const emit = (phase: keyof typeof resizeEventNames, detail: ResizeEventDetail) => {
  ...
  const applyPointerGeometry = (client: SpatialPosition) => {
    ...
    emit("move", resizeDetail(geometry, active.startGeometry, active.edge, "pointer"))
  }
  ...
  const finishPointer = (canceled: boolean, client?: SpatialPosition) => {
    ...
    emit(
      "end",
      resizeDetail(geometry, completed.startGeometry, completed.edge, "pointer", canceled)
    )
  }

  const onPointerDown = (event: PointerEvent) => {
    const handle = matchingHandle(event, root, resizableHandleSelector, resizableRootSelector)
    ...
    active = {
      edge,
      handle,
      limits: resizeLimitsFor(root, edge, boundary, boundarySelector, boundaryPadding),
      pointerId: event.pointerId,
  ...
  const onKeyDown = (event: KeyboardEvent) => {
    const handle = matchingHandle(event, root, resizableHandleSelector, resizableRootSelector)
    const edge = handle ? edgeFromHandle(handle) : undefined
    if (!handle || !edge || event.altKey || event.ctrlKey || event.metaKey) return

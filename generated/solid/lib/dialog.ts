...
function trapFocus(event: KeyboardEvent, element: HTMLElement) {
  ...
  if (event.shiftKey && (active === first || !element.contains(active))) {
    event.preventDefault()
    last?.focus()
  } else if (first && !event.shiftKey && (active === last || !element.contains(active))) {
    event.preventDefault()
    first.focus()
  }
}
...
export function onDialogDismiss(
  ...
  const handler: EventListener = (event) => listener(event as DialogDismissEvent)
...
export function connectDialog(element: HTMLElement): DialogController {
  ...
  const activate = () => {
    ...
    const active = element.ownerDocument.activeElement
    restoreFocusTo =
      active instanceof element.ownerDocument.defaultView!.HTMLElement ? active : undefined
    ...
    queueMicrotask(() => {
      if (layer?.isTop && !element.hidden && element.isConnected) focusDialog(element)
    })
  }

...
function parseDate(value: string): DateParts | undefined {
  ...
  return parts.year === year && parts.month === month && parts.day === day ? parts : undefined
}
...
export function calendarWeekStartsOn(locale?: string): number {
  try {
    const localeObject = new Intl.Locale(normalizeLocale(locale)) as Intl.Locale & {
...
function normalizeWeekStartsOn(value: number | undefined, locale?: string): number {
...
export function formatCalendarDate(value: string, locale?: string): string {
  const parts = parseDate(value)
  if (!parts) return value
  return dateFormatter(normalizeLocale(locale), { dateStyle: "medium" }).format(partsToDate(parts))
}
...
export function formatCalendarValue(
  ...
  return value.end ? `${start} – ${formatCalendarDate(value.end, locale)}` : `${start} –`
}
...
function deserializeDisabledDates(value: string | undefined): CalendarDisabledDate[] {
  if (!value) return []
  try {
...
function selectionState(
  ...
  if (!value) return { inRange: false, rangeEnd: false, rangeStart: false, selected: false }
...
function viewMonth(options: CalendarViewOptions): string {
  ...
  const value = normalizeCalendarValue(options.value, options.selectionMode ?? "single")
  const selectedDate = typeof value === "string" ? value : value?.start
  return selectedDate?.slice(0, 7) ?? defaultCalendarMonth()
}

export function createCalendarView(options: CalendarViewOptions = {}): CalendarView {
  ...
  const heading = dateFormatter(locale, { month: "long", year: "numeric" }).format(
    partsToDate(parseDate(monthStart)!)
  )
  const weekdays = Array.from({ length: 7 }, (_, index): CalendarWeekday => {
    ...
    return {
      label: dateFormatter(locale, { weekday: "narrow" }).format(partsToDate(parsed)),
      longLabel: dateFormatter(locale, { weekday: "long" }).format(partsToDate(parsed)),
    }
  })
...
export function calendarControllerFor(element: HTMLElement): CalendarController | undefined {
...
function elementsForSlot<T extends HTMLElement>(root: HTMLElement, slot: string): T[] {
  return Array.from(root.querySelectorAll<T>(`[data-slot='${slot}']`)).filter((element) =>
    belongsToRoot(element, root)
  )
}

function firstForSlot<T extends HTMLElement>(root: HTMLElement, slot: string): T | undefined {
...
export function connectCalendar(root: HTMLElement): CalendarController {
  const dayButtons = () => elementsForSlot<HTMLButtonElement>(root, "calendar-day")
  const weekdayLabels = () => elementsForSlot<HTMLElement>(root, "calendar-weekday")
  const heading = firstForSlot<HTMLElement>(root, "calendar-heading")
  const previousButton = firstForSlot<HTMLButtonElement>(root, "calendar-previous")
  const nextButton = firstForSlot<HTMLButtonElement>(root, "calendar-next")

  if (!heading || !previousButton || !nextButton || dayButtons().length !== 42) {
  ...
  const minimum = () => (isCalendarDate(root.dataset.min) ? root.dataset.min : undefined)
  const maximum = () => (isCalendarDate(root.dataset.max) ? root.dataset.max : undefined)
  const disabledDates = () => deserializeDisabledDates(root.dataset.disabledDates)
  const today = () => (isCalendarDate(root.dataset.today) ? root.dataset.today : calendarToday())
  const weekStartsOn = () => {
    const value = Number(root.dataset.weekStartsOn)
    return root.dataset.weekStartsOn !== undefined && Number.isInteger(value) ? value : undefined
  }

  let selectedValue = deserializeCalendarValue(root.dataset.initialValue, selectionMode())
  ...
  let focusedDate =
    ...
    (today().slice(0, 7) === visibleMonth ? today() : startOfCalendarMonth(visibleMonth))
  ...
  function focusableDate(date: string, direction: number): string {
    ...
    for (let attempt = 0; attempt < 370; attempt += 1) {
      if (!isDateDisabled(candidate, minimum(), maximum(), disabledDates())) return candidate
  ...
  function updatePreview(buttons: HTMLButtonElement[]) {
    ...
    const start = previewing ? (previewDate! < range.start ? previewDate! : range.start) : undefined
    const end = previewing ? (previewDate! < range.start ? range.start : previewDate!) : undefined
  ...
  function render() {
    ...
    previousMonthButton.disabled = Boolean(
      minimum() &&
      endOfCalendarMonth(addCalendarMonths(`${visibleMonth}-01`, -1).slice(0, 7)) < minimum()!
    )
    nextMonthButton.disabled = Boolean(
      maximum() &&
      startOfCalendarMonth(addCalendarMonths(`${visibleMonth}-01`, 1).slice(0, 7)) > maximum()!
    )
  }

  function focusRenderedDate(date: string) {
    const button = dayButtons().find((candidate) => candidate.dataset.date === date)
  ...
  function handlePointerOver(event: PointerEvent) {
    ...
    if (!(target instanceof Element)) return
    const button = target.closest<HTMLButtonElement>(dayButtonSelector)
    if (!button || !belongsToRoot(button, root) || !button.dataset.date || button.disabled) return
  ...
  const controller: CalendarController = {
    ...
    setValue(value: CalendarValue) {
      ...
      const selectedDate = typeof selectedValue === "string" ? selectedValue : selectedValue?.start

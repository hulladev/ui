...
export const DATE_TIME_PICKER_VALUE_CHANGE_EVENT = "hulla-date-time-picker-value-change"
...
function connectPickerParts(root: HTMLElement, prefix: string) {
  ...
  const handleToggle = () => {
    ...
    if (open) {
      queueMicrotask(() => {
        calendar
          .querySelector<HTMLButtonElement>("[data-slot='calendar-day'][tabindex='0']")
...
function valueIsComplete(value: CalendarValue, selectionMode: CalendarSelectionMode): boolean {
...
export function connectDatePicker(root: HTMLElement): DatePickerController {
  ...
  const singleInput = elementForSlot<HTMLInputElement>(root, "date-picker-input")
  const startInput = elementForSlot<HTMLInputElement>(root, "date-picker-start-input")
  const endInput = elementForSlot<HTMLInputElement>(root, "date-picker-end-input")
  ...
  function update() {
    ...
    const formatted = formatCalendarValue(selectedValue, selectionMode(), pickerLocale(root))
    ...
    if (singleInput) singleInput.value = typeof selectedValue === "string" ? selectedValue : ""
    if (startInput) {
      startInput.value =
        selectedValue && typeof selectedValue !== "string" ? selectedValue.start : ""
    }
    if (endInput) {
      endInput.value =
        selectedValue && typeof selectedValue !== "string" ? (selectedValue.end ?? "") : ""
    }
  }
  ...
  function handleCalendarValueChange(event: Event) {
    ...
    update()
    root.dispatchEvent(
      new CustomEvent<DatePickerValueChangeDetail>(DATE_PICKER_VALUE_CHANGE_EVENT, {
        bubbles: true,
        detail: { value: selectedValue },
      })
    )
    if (valueIsComplete(selectedValue, selectionMode())) hidePopover(pickerParts.surface)
  }
  ...
  return {
    destroy() {
      destroyed = true
      pickerParts.destroy()
      root.removeEventListener(CALENDAR_VALUE_CHANGE_EVENT, handleCalendarValueChange)
...
export function connectDateTimePicker(root: HTMLElement): DateTimePickerController {
  const parts = connectPickerParts(root, "date-time-picker")
  const timePicker = elementForSlot<HTMLElement>(root, "time-picker")
  const timePickerTrigger = elementForSlot<HTMLButtonElement>(root, "time-picker-trigger")
  const applyButton = elementForSlot<HTMLButtonElement>(root, "date-time-picker-apply")
  const formInput = elementForSlot<HTMLInputElement>(root, "date-time-picker-input")

  if (!parts || !timePicker || !timePickerTrigger || !applyButton || !formInput) {
    root.dataset.unsupported = "missing-part"
    return {
      destroy: () => undefined,
      getValue: () => null,
  ...
  let draft: {
    ...
    time: localDateTimeParts(initialValue).time ?? root.dataset.defaultTime ?? "09:00",
  }
  let destroyed = false
  const form = formControl.form

  function candidateValue(): string | null {
    const candidate = draft.date && draft.time ? `${draft.date}T${draft.time}` : null
  ...
  function handleTimeValueChange(event: Event) {
    const value = (event as CustomEvent<TimePickerValueChangeDetail>).detail.value
    event.stopPropagation()
  ...
  function handleApply() {
    ...
    update()
    root.dispatchEvent(
      new CustomEvent<DateTimePickerValueChangeDetail>(DATE_TIME_PICKER_VALUE_CHANGE_EVENT, {
        bubbles: true,
        detail: { value: selectedValue },
      })
    )
  ...
  root.addEventListener(CALENDAR_VALUE_CHANGE_EVENT, handleCalendarValueChange)
  timePickerControl.addEventListener(TIME_PICKER_VALUE_CHANGE_EVENT, handleTimeValueChange)
  ...
  return {
    destroy() {
      ...
      pickerParts.destroy()
      root.removeEventListener(CALENDAR_VALUE_CHANGE_EVENT, handleCalendarValueChange)
      timePickerControl.removeEventListener(TIME_PICKER_VALUE_CHANGE_EVENT, handleTimeValueChange)

...
function groupElements(root: HTMLElement): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>("[data-slot='select-group']")).filter(
    (group) => belongsToRoot(group, root)
  )
}

function valueElements(root: HTMLElement): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>("[data-slot='select-value']")).filter(
    (value) => belongsToRoot(value, root)
  )
}
...
export function connectSelect(root: HTMLElement): SelectController {
  const triggerElement = root.querySelector<HTMLButtonElement>("[data-slot='select-trigger']")
  const contentElement = root.querySelector<HTMLElement>("[data-slot='select-content']")
  const bridgeElement = root.querySelector<HTMLSelectElement>("[data-slot='select-native-control']")
  ...
  if (!content.id) {
    selectContentId += 1
    content.id = root.id ? `${root.id}-content` : `hulla-select-content-${selectContentId}`
  }
  ...
  function showContent(focus: "first" | "last" | "selected" = "selected") {
    ...
    const selected = enabled.find((option) => selectedValues.includes(optionValue(option)))
    const target =
      focus === "first" ? enabled[0] : focus === "last" ? enabled.at(-1) : (selected ?? enabled[0])
  ...
  function refresh() {
    ...
    groupElements(root).forEach((group, index) => {
      ...
      ).find((candidate) => candidate.closest("[data-slot='select-group']") === group)
      if (!label) return
  ...
  function onKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      event.preventDefault()
      focusOption(nextEnabledOption(options(), activeOption, event.key === "ArrowDown" ? 1 : -1))
      return
    }
    ...
    const searchOrder =
      activeIndex === -1
        ? enabled
        : [...enabled.slice(activeIndex + 1), ...enabled.slice(0, activeIndex + 1)]

...
function optionElements(root: HTMLElement): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>(optionSelector)).filter((option) =>
    belongsToRoot(option, root)
  )
}

function groupElements(root: HTMLElement): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>("[data-slot='combobox-group']")).filter(
    (group) => belongsToRoot(group, root)
  )
}

function emptyElements(root: HTMLElement): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>("[data-slot='combobox-empty']")).filter(
    (empty) => belongsToRoot(empty, root)
  )
}
...
export function connectCombobox(root: HTMLElement): ComboboxController {
  const inputElement = root.querySelector<HTMLInputElement>("[data-slot='combobox-input']")
  const contentElement = root.querySelector<HTMLElement>("[data-slot='combobox-content']")
  ...
  if (!content.id) {
    comboboxContentId += 1
    content.id = root.id ? `${root.id}-content` : `hulla-combobox-content-${comboboxContentId}`
  }
  ...
  function updateInputDisplay() {
    ...
    input.dataset.state = editing && query ? "query" : displayValue ? "value" : "placeholder"
  ...
  function normalizeValue(value: ComboboxValue): string[] {
    ...
    const normalized = requested.filter(
      (candidate, index) =>
        candidate !== "" && available.has(candidate) && requested.indexOf(candidate) === index
    )
  ...
  function filterOptions() {
    ...
    options().forEach((option) => {
      option.hidden =
        normalizedQuery !== "" && !optionText(option).toLocaleLowerCase().includes(normalizedQuery)
    })

    groupElements(root).forEach((group) => {
      const groupOptions = Array.from(group.querySelectorAll<HTMLElement>(optionSelector)).filter(
  ...
  function hideContent() {
    if (typeof content.hidePopover === "function" && isOpen()) content.hidePopover()
    editing = false
    query = ""
    filterOptions()
  ...
  function refresh() {
    ...
    groupElements(root).forEach((group, index) => {
      const label = Array.from(
        group.querySelectorAll<HTMLElement>("[data-slot='combobox-group-label']")
      ).find((candidate) => candidate.closest("[data-slot='combobox-group']") === group)
      if (!label) return
  ...
  function onKeyDown(event: KeyboardEvent) {
    ...
    const active = activeId ? enabled.find((option) => option.id === activeId) : undefined
  ...
  function onPointerMove(event: PointerEvent) {
    ...
    if (!(target instanceof Element)) return
    const option = target.closest<HTMLElement>(optionSelector)
    if (option && belongsToRoot(option, root) && !option.hidden && !isDisabledOption(option)) {
  ...
  function onBlur() {
    queueMicrotask(() => {
      ...
      if (!active || (!root.contains(active) && !content.contains(active))) hideContent()
    })
  }
